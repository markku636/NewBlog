...
在 Windows 的 Docker desktop 可以輕鬆透過Docker desktop 勾選 "Expose daemon on tcp://localhost:2375 without TLS" 來啟用2375 port來管理遠端的 Docker 伺服器，但在Linux 版本是沒有這個設定的，如果先裝了 Docker desktop 可能會遇到和我一樣找不到 dockerd 的狀況。
