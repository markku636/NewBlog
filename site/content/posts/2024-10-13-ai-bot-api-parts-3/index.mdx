...
為了避免 AI 胡亂生成，因此，我們要使用向量資料庫，讓AI 擁有其他方面的知識，並提前告知告知大型語言模式，不知道就不要任意生成。

### 首先，我們了解什麼是向量資料庫
向量資料庫是用於存多維度的資料，由大型語言模型計算座標(points)，儲放在向量資料庫，資料與資料間的距離，就能夠推算出相似度。  
...
## 向量化的資料包含什麼 ?
在大一串數值中，它保留原資料的關鍵特徵、維度、距離度量、權重分配、上下文的關聯。

### Langchin 如何操作 向量資料庫
取得各種資料來源(Source) > 加載到應用程式(Load)  > 轉換 (Transform)，切割擷取成比較小的段落  > Embed (向量化) > 儲存到向量資料 (Store) > 取回(Retrieve)
...
## 安裝及用程式操作向量資料庫
### 什麼是 qdrant
Qdrant 是一個向量數據庫，用於儲存和檢索高維度向量資料，適合做相似度查詢，例如推薦系統和文字、圖像搜索，它有簡單易用的 RESTful API 和可視化儀表板來管理和查看向量數據。
...
```
### 透過 Llama 去 Embedding 新增資料到 Collection 
```
import { OllamaEmbeddings } from '@langchain/ollama';
...
def product_recommendation(query: str):
    ...
    try:
        ...
        # product_vectors = generate_product_vectors(df)
        # initialize_qdrant(product_vectors)
        ...
        print(f"在推荐产品时发生错误: {e}")
        return "技术错误，请稍后再试。"
        
        
def search_products(query):
    embeddings = AzureOpenAIEmbeddings(azure_deployment='embedding-3', openai_api_version='2024-02-15-preview')
    query_vector = embeddings.embed_query(query)
    
    client = QdrantClient(path="db")  # 替换为你的Qdrant路径
...
```
## 最近使用 AI 的心得
最近蠻有感的，AI 出現確實拿走了一些基礎工作，讓新的工程師，進入職場更辛苦，讓資深透過用ＡI，可以做更多的事;隨著各種知識學習門檻變得簡單，很多知識，可以透過和AI 聊天就學會，人們獲取知識也更容易，我們花更多時間擷取知識、洞察、規劃、驗證及下決策。

## 參考資料
* [Qdrant 向量資料庫基本練習](https://blog.darkthread.net/blog/qdrant-w-cs/)
* [使用Qdrant向量資料庫實作語意相似度比對
](https://studyhost.blogspot.com/2024/04/qdrant.html)
