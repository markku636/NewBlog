---
featured: true
title:  優化舊系統佈署流程 - 改善 NET MVC 自動化建置，手動佈署
date:  2024-09-03 01:01:35 +0800
thumbnail:  jenkins.png
category:  DevOps  
tags:   ['special-technical-problems']
description : 優化舊系統佈署流程 - 改善 NET MVC 自動化建置，手動佈署
author : Mark Ku
slug: net-mvc-auto-build-manual-deploy
---
## 時空背景
大部的系統皆能夠容器化，但總有些網站最舊也最大，暫時無法做技術昇級，但人數一多手動佈版，都會遇到有人漏Commit binary，因此想把自動建置的部份自動化，減化佈版流程也減少錯誤帶來的影響。  

因為是舊系統及Net Mvc的舊框架，無法容器化，全自動佈版，會有點風險，因此這邊佈署的部份，仍然還是採用手動。

## 目標
* Jenkins 自動建置，自動commit 推送到 Github 
* 生產環境透過 beyond compare 比較 binary 差異，手動佈版
...
```
## 最後，執行後就能透過，Beyond compare 去比較此次佈版的dll 是不是和所想的一樣。
