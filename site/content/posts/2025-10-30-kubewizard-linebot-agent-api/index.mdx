...
支援以插件形式擴充工具（LangChain Tools），例如：
...
- 使用 Redis 儲存對話歷史，支援上下文理解
- 自動總結長對話，避免 Token 超限
- 每個用戶擁有獨立的 session，互不干擾
...
很多人第一次接觸 LangChain 會覺得「好像很複雜」，其實它就是幫你把「LLM + 工具 + 記憶 + 提示詞 + 執行流程」這些零件組成一個可擴充的腦袋。以下用生活比喻快速理解：

| 概念 | 在 KubeWizard 裡 | 比喻 |
|------|------------------|------|
| LLM (大腦) | `ChatGoogleGenerativeAI` | 顧問的大腦，負責理解問題與思考 |
| Prompt (說明書) | `SYSTEM_PROMPT` + `ChatPromptTemplate` | 給顧問的人設與工作流程 SOP |
| Tools (工具箱) | `KubeTool`, `RequestsGet`, `search`, `human_console_input` | 顧問手邊的螺絲起子、手冊、電話詢問管道 |
| Memory (記憶) | `RedisChatMessageHistory` + `ConversationTokenBufferMemory` | 顧問的筆記本（會自動刪舊做摘要）|
| Agent (決策迴路) | `create_openai_tools_agent` + `AgentExecutor` | 顧問決定要不要翻工具箱、查筆記還是直接回答 |
| Scratchpad | `agent_scratchpad` | 顧問的「思考過程便條紙」 |

工作流程超簡化：
1. 使用者輸入問題
2. Prompt + 記憶提供上下文給 LLM
3. LLM 判斷是否需要工具（例如列出 pods → 用 KubeTool）
4. 執行工具取得結果（Pod 狀態 / HTTP 回應）
5. LLM 整合工具輸出形成最終答覆
6. 記憶系統儲存本輪對話，必要時做摘要避免變太胖
...
小技巧分享：
- Prompt 中明確列出常說口頭禪 → 模型回答更一致。
- 工具描述要「使用者語言」而非僅程式語言，例如：`"查詢 Kubernetes Pods（可附加 namespace）"`。
- 危險操作要分工具並加人工確認 → 避免模型過度自信。
- 日誌層級分兩段：DEBUG 看推理、INFO 看工具使用。
...
- ✅ **不依賴 CLI**：不需安裝 kubectl 二進制文件
- ✅ **Docker 映像更小**：從 ~500MB 減少到 ~300MB（節省 40%）
- ✅ **更安全**：避免字串注入攻擊與 shell 執行風險
- ✅ **更好的錯誤處理**：Python SDK 提供結構化的異常
- ✅ **程式化控制**：可精準控制每個 API 調用
- ✅ **RBAC 整合**：可搭配 ServiceAccount 精準控管權限

Python SDK 在 **Pod 內與外部皆可使用**：

| 運行環境 | 載入方式 | 配置來源 | 用途 |
|---------|--------|--------|--------|
| 叢集內（Pod） | `load_incluster_config()` | ServiceAccount Token | 正式部署與權限控管 |
| 外部（本地/VM） | `load_kube_config()` | ~/.kube/config | 開發與測試 |
...
def load_k8s_config():
    """自動判斷環境並載入適當的 K8s 配置"""
    try:
        # 檢查是否在 K8s Pod 內（存在 ServiceAccount Token）
...
def list_pods(namespace="default"):
    """列出指定 namespace 的所有 pods"""
    try:
        pods = v1.list_namespaced_pod(namespace=namespace)
        return [pod.metadata.name for pod in pods.items]
    except client.exceptions.ApiException as e:
        logger.error(f"API 錯誤: {e.status} - {e.reason}")
        return []
```

**支援的 kubectl 命令轉換：**

| kubectl 命令 | Python SDK 實現 |
|-------------|----------------|
| `kubectl get pods -n default` | `v1.list_namespaced_pod(namespace="default")` |
| `kubectl get nodes` | `v1.list_node()` |
| `kubectl describe pod <name> -n <ns>` | `v1.read_namespaced_pod(name, namespace)` |
| `kubectl logs <pod> -n <ns>` | `v1.read_namespaced_pod_log(name, namespace)` |
| `kubectl get deployments -n <ns>` | `apps_v1.list_namespaced_deployment(namespace)` |
...
mysql-5.7-6c9d4f...          1/1     Running   1          10d

總計: 3 pods (3 Running, 0 Pending, 0 Failed)
```
