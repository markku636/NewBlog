...
剛進來這公司時 Next js 剛上線，頁面一直拿到舊的，我也才來一個月左右，被美國的 User 念了一小時，且因為美國的銷售檔期(Holiday season)要來了，所以這個問題有時限性，且當時，我對Next js 沒到很熟。  

彙總我遇到兩個大問題  

## 問題一、增量渲染的頁面(ISR)，一直拿舊的頁面，超過 revalidate，再訪問，頁面仍然沒重新渲染。
...
為了讓使用者快速拿到渲染的頁面內容，我們當時採用的是增量渲染 ISR ，但面臨銷售檔期，很容易遇到 Sold out ，明明庫存清單及明細頁都顯示有庫存，但加入購物車就會出現庫存不足。
