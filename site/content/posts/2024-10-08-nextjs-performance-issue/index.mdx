...
當時我們的頁面流量都不小，每天都超過3萬流量，發現佈署一陣子的 docker 容器的大小，都會從2gb 成長到 8gb，經過分析調查，發現我們user 的上傳原始圖檔過大，且都沒壓縮過，最大有10MB，猜測 node js 因為是單執行緒，只透過 event loop 在調度事件，一邊在轉圖， 一邊在渲染，推測 next js 的轉檔效能問題，
