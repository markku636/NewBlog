...
## 安裝Dashboard 及產生 ServiceAccount 及 取得Token (這個token 是要拿去給jenkins 用的)
### 安裝 Dashboard
```
...
pipeline {
    ...
    stages {
		...
		stage("Delete deployment") {
			steps {				
                sh 'kubectl delete deployment <your-deployment-name> --ignore-not-found=true'				
			}
		}				
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: nextjs
        image: 192.168.50.50:5000/next-ec:latest
        ports:
        - containerPort: 80
        resources:
          limits:
