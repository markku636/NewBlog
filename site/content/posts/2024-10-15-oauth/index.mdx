...
* OAuth 有好幾種 type ，
* Code  => 多打一次 request。
* Token => 比較方便，但所有東西都藏在後端。
* Client Credentials Grant =>客戶端憑證模式， 服務A想訪問服務B的API，服務A使用自己的客戶端憑證獲取訪問令牌。
* Implicit Grant=> 透過 Javascript ，但因為安全性問題逐漸被廢棄。
* Resource Owner Password Credentials Grant:直接和用戶端要帳密，安全性較低現在比較沒在用。
...
* [官方文件](https://oauth.net/2/token-exchange/)