---
featured: true
title: 從地端到雲端：使用 Google Kubernetes Service 部署你的Next.js 應用
date:  2024-11-13 01:01:35 +0800
thumbnail:  gks.png
category:  Cloud  
tags:   ['Google Cloud','Docker','Artifact Registry','kubernetes','GKS']
description : 從地端到雲端：使用 Google Kubernetes Service 部署你的Next.js 應用
author : Mark Ku
slug: deploy-nextjs-app-to-google-kubernetes-service
---
### 前言
先前[有透過 Cloud Run 運行容器應用](https://blog.markkulab.net/on-prem-to-cloud-gcp-docker-deployment-artifact-registry-cloud-run/)，也有試著在[地端架了Kubernetes](https://blog.markkulab.net/enable-docker-2375-port-in-ubuntu22/)，這篇來談 GKS (google kubernetes service)，目前市面上的雲服務，都是基於 Kubernetes 的雲端架構服務，如:AWS、AKS、GKS，因此 Kubernetes的基礎知識就顯得很重要，但Kubernetes 的官方文件也真的很多東西，只好邊實作邊學習。
...
**配置 Load Balancer**
自動建立 Load Balancer 外部 IP 和防火牆規則：
```bash
...
```
[相關文件](https://kubernetes.io/docs/tasks/access-application-cluster/create-external-load-balancer/)
