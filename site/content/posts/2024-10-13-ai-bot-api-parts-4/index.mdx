...
### 但在真實的世界裡，水餃和動物，並不太相關，資料間的相似度是模型透過特定演算法得出，看起來是不能修改，因此，必須透過其他方式解決查詢精準度的問題，以下是我整理出可行的方式
 
* 1. 寫入向量資料庫時，加入結構化的特徵，在查詢時加入類別，並利用篩選條件去查詢
```
 qdrantClient.filter({
    ...
    filter: {
        must: [
            ...
            { key: 'category', match: { value: 'Electronics' } } // 分類查詢
        ]
    },
    limit: 5
});
