...
### 接著點選 OPEN GRAPH 按鈕，就能展開被選取資料的關係圖，並將滑鼠滑動到連接的線上，就能得知，資料與資料間的相似度。
![Graph](qdrant-dashbaord-3.png)

P.S. 發現有趣像現是的是水餃和動物相似度居然有 0.8，和HP筆電的相似度是0.06，不知道是不是因為水餃是用動物做的嗎? XD 

### 但真實的水餃和動物，並不太相關，但相似度是模型透過特定演算法得出，看起來是我們不能修改，因此，必須透過其他方式解決查詢精準度的問題，以下是我整理出可行的方式
 
* 寫入向量資料庫時，加入特徵，在查詢加入類別，並利用篩選條件去查詢
```
{
  "category": "筆記本電腦"
}
...
```
### 插入大量產品資料 (/api/lang-chain/insert-products-vector-database)
```
...
```
查詢產品資料 (/api/lang-chain/query-product-vector-database)
```
import { OllamaEmbeddings } from '@langchain/ollama';
import { QdrantClient } from '@qdrant/js-client-rest'; // 引入 Qdrant 客戶端
import { NextApiRequest, NextApiResponse } from 'next';
...
### 執行API
```
curl -X POST http://localhost:3001/api/lang-chain/query-product-vector-database \
  -H "Content-Type: application/json" \
  -d '{"text": "幫我推薦一台筆電"}'
```

## 加入查條件 Category，所查出來的結果，就更精準了
```
{
  "results": [
    {
...
```
## 結論
依據前面幾篇的實驗，因此我們得出，如何用Langchain 打造更精準的產品推薦的AI代理機器人。  

使用者輸入 > 解析用戶的意圖 > 依據動態路由，AI決定採用什麼工具 > 執行產品推薦工具 > AI依據使用者解析產品分類，組成向資料庫查詢條件 > 查詢向量資料庫 > 回傳結果給最終用戶。
