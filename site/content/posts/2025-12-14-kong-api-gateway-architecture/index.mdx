...
客戶擁有大量內部 API 服務，希望透過數位轉型將其**對外開放並商業化**。核心目標是建立一個**自助式 API 租賃平台**，讓開發者能快速申請、按量付費使用。
...
透過 API Gateway，我們實現了：

- **效率提升**：API Key 申請從 好幾天縮短至幾分鐘
...
### 在設計對外開放的認證機制時，我們在 **JWT**、**HMAC** 與 **API Key** 之間進行了深度權衡。考量到目標客群（包含學生、學術單位及地圖開發者）的開發能力，以及與 GIS 工具（如 QGIS）的相容性，我們最終選擇了 **Key Auth** 方案：

* **排除 JWT**：雖然安全性高，但開發者需處理 Token 過期、刷新機制，會大幅增加客戶端的串接複雜度。
* **排除 HMAC**：需要複雜的簽章演算法與加解密流程。考量到用戶群可能缺乏相關開發背景，我們不希望為了認證而需提供各語言的 SDK 或範例程式，增加維運負擔。
* **選擇 Key Auth (Header)**：
* **低門檻**：僅需在 HTTP Header 中帶入金鑰，極易上手。
* **高相容性**：大多數地理資訊系統軟體（如 **QGIS**）或 Web 地圖框架（OpenLayers, Leaflet）對 Header 驗證支援度最佳。
* **安全加固**：針對商業用戶等高安全性需求，我們額外利用 **IP Restriction** 插件，透過 **IP 白名單綁定** 來彌補 API Key 的安全風險，確保金鑰即便外洩，非授權來源也無法存取。
...
## 6. 心得
Kong OSS 本身不提供群組規則、進階流量控管或僅限 route 層級的控制機制，也沒有 API 訂閱的原生概念。因此，若要使用 Kong 來實作 API 訂閱制或較為複雜的群組權限與流量規則，通常需要進行大量客製化；否則在設計上容易顯得不夠貼合需求，必須遷就既有的架構與邏輯，湊起來運費就會像卡卡的，且在這過程我也更了解，比起接案我更喜歡做產品或平台，時間上會比較充裕，且會更考量品質及維護性。