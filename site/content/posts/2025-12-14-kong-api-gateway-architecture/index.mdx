---
title: "半年回顧：評估大型圖資系統 API Gateway (Kong) 導入與架構實踐"
date: "2025-12-14"
category: "DevOps"
tags:
  - API Gateway
...
graph LR
    ...
    subgraph Rate_Limit [頻次控制]
        Type --> |向量圖資| RL_Basic[基礎頻次]
        Type --> |API 服務| RL_High[高頻次通道]
    end
    
    subgraph Bandwidth_Limit [流量/頻寬控制]
        Type --> |3D 模型| BW_Heavy[大流量寬限]
    end
    
    RL_Basic --> Redis[(Redis Cluster)]
    RL_High --> Redis
    BW_Heavy --> Redis
```
