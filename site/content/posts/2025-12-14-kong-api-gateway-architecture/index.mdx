...
在這過去的半年裡，我深度參與了「圖資開放平台」的建置案，主要負責第一期的 **API Gateway 架構評估及驗證**。
...
### 路由與服務配置
我利用 **Kong Manager** 進行了標準化的配置工作，重點如下：
...
完善的監控是 Gateway 穩定運行的基石。這半年我投入了大量心力整合 **ELK** 與 **Prometheus** 生態系，實現從「日誌查詢」到「指標儀表板」的全方位可觀測性。
...
在驗收過程中，我們使用開源 API 測試工具 **Hoppscotch** (前身為 Postwoman) 針對 15 項關鍵服務進行了密集的功能驗證。測試結果顯示，API Gateway 不僅能有效攔截未授權的惡意請求，其精細的流量控制機制也能在系統高負載下維持服務穩定，成功達成契約所訂定的各項效能指標。
