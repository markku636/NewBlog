...
客戶擁有大量內部 API 服務，希望透過數位轉型將其**對外開放並商業化**。核心目標是建立一個**自助式 API 租賃平台**，讓開發者能快速申請、按量付費使用，第一期的部份，就是評估方案和架構在正式開發系統前，是否能夠達成需求。
...
在設計對外開放的認證機制時，我們在 JWT、HMAC 與 API Key 之間進行了深度權衡。考量到目標客群（包含學生、學術單位及一般開發者）的整體開發能力，以及各類客戶端工具與系統的相容性，我們最終選擇了 Key Auth 方案：

排除 JWT：雖然安全性高，但開發者需處理 Token 過期與刷新機制，會顯著增加客戶端整合與維運的複雜度。

排除 HMAC：需實作簽章演算法與加解密流程。考量到用戶背景差異，我們不希望因認證機制而必須提供各語言的 SDK 或範例程式，進一步提高使用與維護成本。

選擇 Key Auth（Header）： 低門檻：僅需於 HTTP Header 中攜帶金鑰即可完成驗證，學習與導入成本最低。

高相容性：多數常見的客戶端工具、第三方服務與應用框架皆原生支援 Header 驗證方式。

進階的安全性：更高高安全需求場景，可額外搭配 IP Restriction 機制，透過 IP 白名單綁定 降低 API Key 外洩風險，確保僅授權來源可存取服務。
...
## 6. 心得
Kong OSS 本身不提供群組規則、進階流量控管或僅限 route 層級的控制機制，也沒有 API 訂閱的原生概念。因此，若要使用 Kong 來實作 API 訂閱制或較為複雜的群組權限與流量規則，通常需要進行大量客製化；否則在設計上容易顯得不夠貼合需求，必須遷就既有的架構與邏輯，湊起來運費就會像卡卡的，且在這過程我也更了解，比起接案我更傾向做產品或平台，時間上會比較充裕，且會更考量品質及長期可維護性。