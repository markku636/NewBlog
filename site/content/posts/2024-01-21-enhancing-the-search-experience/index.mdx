...
| 查詢條件類型           | 描述                                                                                          | 範例                        |
|--------------------|-----------------------------------------------------------------------------------------------|-----------------------------|
| **`match`**     | 用於全文檢索，對搜尋詞進行分詞，然後搜索每個分詞，支援文字字段的分析，適用於搜索文字字段。 | 搜尋 "快速的電腦"          |
| **`match_phrase`** | 也用於全文檢索，但要求匹配整個詞組，考慮詞序，適用於精確詞組匹配的情況。               | 搜尋 "快速的電腦"，但要求匹配整個詞組|
| **`multi_match`** | 允許在多個字段中進行 `match` 查詢，適用於在多個字段中搜索相同關鍵字的情況。           | 在標題和描述中搜尋 "蘋果"     |
