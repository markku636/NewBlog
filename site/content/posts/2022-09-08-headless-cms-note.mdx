...
## 解決問題
老闆請我協助評估 CMS 讓產品部門維護 FAQ 及產品 KB ，因為開發能量有些限、希望能靠友善的 UI 設定，就能快速的創造出後台維護頁面及前台Api，好讓其他前端同仁串接。
...
我試用了市面上幾套主流 CMS，基於以下理由，首推 strapi 靈活輕巧，適合企業中長期發展，未來也可以移作他用。   
使用者 - 界面友善、可以自由調整位置。  
資料管理、角色權限、流程控制，是其他套達不到的。  
...
## Api 沒回傳圖片
API 預設是不回傳圖片的欄位，需要額外 populate 需額外填寫回傳的圖片欄位
