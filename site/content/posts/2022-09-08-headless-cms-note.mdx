---
featured: true
title: Headless CMS Strapi 筆記
date:  2022-09-08 01:01:01 +0800
thumbnail: strapi.png
category: Infra
tags: ['headless','cms','strapi','swagger','faq']
description :  Headless CMS Strapi 筆記
author : Mark Ku
---

## 解決問題
老闆請我協助評估 CMS 讓產品部門維護 FAQ ，因為開發能量有些限、希望能靠友善的 UI 設定，就能快速的創造出後台維護頁面及前台Api。

## 原理
透過友善的 UI ，程式產生程式。
![](https://i.imgur.com/M6RfjZ8.png)

## 為何採用 strapi
![](https://i.imgur.com/ONXbTAJ.jpg)
我試用了上面幾套 CMS，基於以下理由，首推 strapi 靈活輕巧，適合企業中長期發展，未來也可以移作他用。   
使用者 - 界面友善、可以自由調整位置。  
管理者 - 資料、權限、流程控制。  
開發者 - 開源、串接友善，提供 swagger 文件、擴充性( 插件市集 )、提供豐富的接口、本身也是 react 及 node 寫的，對前端開發者相對很友善。  

## 限制
* node 版本 '>=10.16.0 <=14.x.x' ( 可以透過 nvm 管理多版本 node )
* 雲端付費，地端開源永久免費

## [安裝 strapi](https://docs.strapi.io/developer-docs/latest/getting-started/quick-start.html#_1-install-strapi-and-create-a-new-project)

```
npx create-strapi-app@latest my-project --quickstart
```
## 安裝 [swagger](https://docs.strapi.io/developer-docs/latest/plugins/documentation.html#installation)  
```
cd my-project
npm run strapi install documentation
```

## Api 沒回傳圖片
API 預設是不回傳圖片的，需要
![](https://i.imgur.com/6gfNF29.png)

![](https://i.imgur.com/eF34HEH.png)

## Api 權限
預設是需要帶 token 存取，如果你不需要驗證則可以，到Setting > USERS & PERMISSIONS PLUGIN > Role > 找到你的 Collection > 並給與 find 和 findOne 權限。
![](https://i.imgur.com/ovdumL9.png)

## Rich Text 圖片無法預覽
.env
```
HOST=0.0.0.0
PORT=1337
APP_KEYS=keys
API_TOKEN_SALT=9LGnsz1zt4UpaKfCCQxEZQ==
ADMIN_JWT_SECRET=secret
JWT_SECRET=mJg/1OqVkOeCiZcxiQfG5w==

------
WEBSITE=http://127.0.0.1:1337/
------
```
config/server.js
```
module.exports = ({ env }) => ({
  host: env('HOST', '0.0.0.0'),
  port: env.int('PORT', 1337),
  ------
  url: env('WEBSITE', 'http://127.0.0.1'), 
  ------
  app: {
    keys: env.array('APP_KEYS'),
  },  
});

```
## 備份
* 資料庫 - 預設是 sql-lite ，路徑 .tmp/data.db ( 也支援 mysql 及 mongo )
* 靜態檔案 - 拷貝 app/public/uploads

## 功能頁面展示
### 動態產生後台

![](https://i.imgur.com/0f0BjwY.png)

### 上傳圖片可以傳多張圖
![](https://i.imgur.com/jXmzMbk.png)
### 上傳圖片格式限制
![](https://i.imgur.com/PunbK7f.png)

## 輸入畫面
![](https://i.imgur.com/lLcJ5rp.png)

### Layout 動態調整
![](https://i.imgur.com/hGjLnld.png)

### WebHook
![](https://i.imgur.com/DAuWE1D.png)

### 角色
![](https://i.imgur.com/i1fy4kz.png)

## 權限
![](https://i.imgur.com/gEvb3eh.png)










## [備份](https://github.com/8byr0/strapi-plugin-backup-restore)
不支援 v3 版

```
npm i strapi-plugin-backup-restore

```

Create a private/backups folder. This folder will be used to store your backups.


```
// config/server.js
module.exports = ({ env }) => ({
  host: env("HOST", "0.0.0.0"),
  port: env.int("PORT", 1337),
  admin: {
    auth: {
      secret: env("ADMIN_JWT_SECRET", "32a00a220c916908e0efca2b8117262f"),
    },
    // Add this line
    watchIgnoreFiles: ["**/private/**"],
  },
});
```