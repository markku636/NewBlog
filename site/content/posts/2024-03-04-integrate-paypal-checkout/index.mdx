...
---
## 時空背景
據[統計](https://www.rapyd.net/blog/ecommerce-and-payment-trends-germany/?fbclid=IwAR0uLqmxcAoefhQMFfvoOPw3OutAOioXDsqRH9BaRusOieVHPZP2n1NDziQ) Paypal 在德國擁有22%以上的市佔，並在歐盟也有一定的市佔，但過去因為詐騙太嚴重，因此把Paypal 關閉，藉由此次重新打造德國網站，把Paypal 重新串接，並搭配 [Riskified](https://www.riskified.com/) 保險服務，來分攤掉網路電商的詐欺風險。
![](riskified.png)

## Paypal js 串接文件
Api 看起來改了版，官方也建議用新版的rest api 來串接
...
* [建立訂單相關的api](https://developer.paypal.com/docs/api/orders/v2/#orders_create)

## 新版相當的方便，可以透過Event logs [除錯](https://developer.paypal.com/dashboard/dashboard/sandbox)

![debug-tools](debug-tools.png)

## 進階付款功能
額外發現進階模式多了，Venmo(社交支付)、Debit or Credit Card、Paypal Pay Later (先買後付)，但需要額外申請 PayPal 的 Braintree。

P.S. PayPal 的 Braintree 是一個全面的支付解決方案，旨在幫助商家接受、處理和分配支付。它解決了多個與接受線上支付相關的問題，提供了一個安全、靈活且易於集成的平台。
...
## 開始串接
### Integration builder 
其實官方的 [Integration builder](https://developer.paypal.com/integration-builder/) 的範例寫的不錯，基本上照著界接就能做完了
...
export default function TestPaypal() {
    ...
    const [Capture] = useCaptureMutation(); // rtk query 用來呼叫提取信用的api
...
## 後端
### PAYPAL API 請求權限 - Authorization paypal 支援，兩種token，這邊採用 <client_id:secret> 當成登入的token 

依據官方文件，所述，我們可以呼叫api 取得 Access-Token 或是使用 client_id:secret 呼叫paypal API
...
public virtual async Task<Result<PaymentResult>> CaptureAsync(GernalPaymentParameter paymentParameter)
{
   ...
   var result = new Result<PaymentResult>
   {
      ...
      Data = new PaymentResult
      {
         PaymentReturnType = PaymentReturnType.Token,
         //PaymentReturnValue = response
      }
   };
...
### 相關文章，授權和提款 
* [美國電子商務實務筆記 - 信用卡授權(Authorize)及拿取(Capture)](https://blog.markkulab.net/usa-ecommerce-note-credit-card-authorize-and-capture/)
