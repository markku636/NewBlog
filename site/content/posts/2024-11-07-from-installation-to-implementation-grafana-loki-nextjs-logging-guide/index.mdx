...
---
## 前言
因為我們有多個國家的網站，原本使用 Seq 作為 log server，但因為同時間只能有一個人登入，每個國家都需要各自的容器服務，增加了運維的複雜度,為了簡化運維，我決定將 log server 換成 Grafana，並在學習 K8s 時發現 Grafana、Loki 和 Prometheus 成為常見的標配。
...
server {
    listen 80;
    location /loki/ {
        proxy_pass http://localhost:3100; # Loki 实例地址
        proxy_set_header Authorization "Bearer YOUR_TOKEN"; # 设置令牌
        # 可根据需求限制访问的 IP、路径等
