...
即使在防火牆上限制 TCP/IP 的連線數，路由器仍然頻繁被打爆，導致辦公室網路多次癱瘓。

## 發現駭客行為

經過深入調查，在Docker Host 中發現有多個異常的Ubuntu的容器在運行，進而得知得知，透過Docker 的遠端管理連接埠，內網的任何人，都能遠端拉取的官方的 Docker 映像檔，並可以連線進容器中做任何的操作，駭客利用這些惡意容器耗盡主機資源，包括網路、CPU 和硬碟，或偷取資料。
![docker ps](docker-ps.png)

## 如何避免類似問題
了解駭客如何利用 Docker 權限過大的漏洞後，我決定加上 TLS 憑證來強化安全性，防止開發機的 Docker 被駭客劫持。在找到蠕蟲或跳板之前，這是一個有效的防護手段。
...
```
![image](https://hackmd.io/_uploads/H1iN1-YBke.png)
