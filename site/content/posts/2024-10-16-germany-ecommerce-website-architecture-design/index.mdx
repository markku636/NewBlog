...
```
### 暫存訂單 ( 用到抽像 abstract class 讓程式覆用 及 這邊並用了自動注入及命名解析依賴反轉，達到金流模組化  )
...
這次的成功不只是個人的努力，過程中也獲得了許多人的幫助，由於沒有專職PM，設計主管和技術主管兼任這個角色，加上資源有限，我們在一邊規劃、一邊開發的同時，還需要協調外部事務，能夠達成這個目標，真的很不容易。
