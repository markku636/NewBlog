...
## 挑戰與困難
經歷了充滿挑戰的第一年半年，美國網站的大問題趨近於穩定，終於可以開始做德國網站，但好幾個老闆們，有不同的大目標，礙於我們開發資源不夠，好幾個專案，全部一起執行會變慢，大家切來切去大家也很痛苦。

我知道好幾個同時進行會變慢，因此我來來回回去溝通幾次，最終德國網站因為是大老闆要的，一級主管們又被加到同個群組裡，最終終於能夠專注開發。

## 德國電商網站 - 架構設計
![架構設計](ａrchitecture.png)

採用的技術是 NextJS App routing + Dotnet core 8 ，我主要負責分析需求、規劃架構、拆解工作清單，前端及後端程式核心的程式撰寫，並獨立建置整套佈署開發環境及正式佈署開發環境。

## 針對技術細節，做了那些設計
## 前端網站( NextJS )
### Proxy api
在過去使用 Next.js 時，每當需要呼叫 C# API，我們都得在 NextJS 裡額外實作一個 API，現在透過使用 http-proxy-middleware 套件客製的代理伺服器 (proxy)，我們不再需要費時維護 NextJS 額外的 API，大幅節省了開發時間及跨域存取的問題。
...
過去採用的Session，其實就不易做到橫向擴展，為了能讓後端API，未來伺服器能橫向擴展，降低api 偶合度，我們這次導入了，Token base（ＪＷＴ）為辨別用戶身份的方式。  
...
## 程式產生器後端有些程式是透過程式產生器
為了加速開發，後端底層是採用開源的框架，並透過［程式產生器］（https://www.donet5.com/Doc/33），建立模型及資料存取層，加速後端的開發。

## Redux toolkit & RTK 做為狀態管理及API查詢
嘗試使用
...
在防止機器人攻擊方面，在登入或是查詢訂單頁面，我們將原本的Google CAPTCHA 替換了 Cloudflare Turnstile，一來可以省錢(免費)，串接也更泂易，且對使用者體驗更好，且比Google CAPTCHA V3又更少誤判。
[Syy7DnEKp](https://hackmd.io/_uploads/H10v0AT11x.png)
...
* 產品頁，使用Azure AI 產 SEO Meta Tag 資料。
* 使用了 Bin 的g Index API Now
* 使用了Ｓｅｒｖｅｒｓｉｄｅ　ｃｏｍｐｏｅｎｔ　提昇了網站效能。

## 單元測試 
此次針對結帳金額或算計算運費，前後後端都有寫測試，避免未來改壞，並依據3A元則進行測試。  
...
## 功能面
### 客服軟體
因為德國網站很小，過去沒有客服軟體，我建議德國公司用了What's app 當客服軟體。
...
![Riskfield](ｒiskfield.png)

## DevOps 遇到的一些比較特別問題
［因為遇到的問題比較多，因此額外寫了一篇ｂｌｏｇ］（https://blog.markkulab.net/germany-website-go-live-encountered-special-technical-problems/）

## 網站排名，剛上線時  4350 到 3745 名
![Ｗｅｂ　Ｒａｎｋ](ｗｅｂｒａｎｋ．ｐｎｇ)

## 結論
真的很不容易，最終德國網站終於順上線，過程中感謝很多人的幫助，在這過程我也學到很多。
