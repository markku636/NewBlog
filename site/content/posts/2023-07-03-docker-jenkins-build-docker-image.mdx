...
## 開發環境說明
預期開發者 Commit Code 後，自動將 Github Repo 拉下來，並自動建置，最後直接佈署到最終的 Windows Docker Desktop 主機。
...
## 因為我比較熟 docker 指令，所以在 Jenkins 容器中安裝 Docker 
```
...
```
## 最後，我們要設定當 Gihtub Commit Push 時 trigger 建置

### 將你的 Jenkins 透過 Cloudflare Tunnel 當反向代理工具，讓Github Push 可以訪問
 ## 至 Github 的 Setting 設定 Webhook 
 ```
 https://{your-domain}/github-webhook/
 ```
![](https://i.imgur.com/J7rqy4H.png)
...
### 複製出備份檔及還原
```
docker cp jenkins:/var/jenkins_home/jenkins-backup/. d:\\jenkins-backup
docker cp D:\\jenkins-backup\\. jenkins:/var/jenkins_home/jenkins-backup/
```
