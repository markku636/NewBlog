---
featured: true
title:  Docker Jenkin + github SSH 憑證 + Build Docker Image 開發環境整合筆記
date:   2023-07-03 01:01:35 +0800
thumbnail:  jenkins.png
category:  PowerShell
tags:   ['jenkins','docker image build','Docker Desktop']
description : Docker Jenkin + github SSH 憑證 + Build Docker Image 整合筆記
author : Mark Ku
slug: reduce-the-risk-of-manual-deployment-using-powershell-and-beyondcompare
---
## 目的
先前用 [Powershell 遠端建置測試機的 DockerDesktop](https://blog.markkulab.net/remote-build-docker-image-with-powershell/)，開發人員一多後，發現太麻煩了，最後還是弄了Jenkin，並整合 Github 憑證及遠端佈署 DockerDesktop

## 開發環境說明
預期開發者 Commit Code 後，自動將 Github Repo 拉下來，並自動建置，掛在到最終的 Docker Desktop 主機。

## 建立 Jenkins 環境

### 首先開啟 Powershell ，建立資料夾作為 Jenkins 的工作目錄
```
...
```
docker run -d -p 8080:8080  -p 50000:50000 -v ${workspacePath}:/var/jenkins_home/workspace --name jenkins --restart=always jenkins/jenkins:lts 
```
### 看到這畫面，要進去容器拿這密碼

![](https://hackmd.io/_uploads/r1FFLzktn.png)
### 接著，安裝建議的插件

![](https://hackmd.io/_uploads/SyNxJ2kYn.png)
### 等待安裝完成，就能使用 localhost:8080 進入 Jenkins 網頁
![](https://hackmd.io/_uploads/ByFaLGJY3.png)
...
## 因為我比較熟 docker 指令，用以下指令在 Jenkins 容器安裝 Docker 
```
