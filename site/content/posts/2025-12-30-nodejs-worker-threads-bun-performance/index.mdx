...
「一顆核心滿載、其他核心在發呆」的畫面，直接指向一件事：**Node.js 的單執行緒模型，已經成為瓶頸。
...
相對地，像 setTimeout、HTTP 請求這類需要等待的操作，並不會直接佔用 call stack。執行環境會先將它們交給底層 API（或 libuv）處理，等任務完成後，再把對應的 callback 放進佇列中，等待 event loop 在 call stack 空閒時依序取回並執行。  
