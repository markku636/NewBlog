---
featured: true
title: 解決 Node.js 單執行緒效能瓶頸：Worker Threads、Cluster 與 Bun 實戰
...
- 根據多個節點的健康度做 **負載平衡**（例如加權輪詢、最少連線數）
- 當主要節點掛掉時做 **Failover**，把流量切到備援節點
- 這些邏輯又會 **隨著監控數量成倍增加** 被重複執行
...
- 所以只要是「等 I/O」為主的工作（API 呼叫、DB 查詢），Node.js 通常都能跑得很順
...
- 重要度分級：核心監控維持較短 interval，一般監控拉長。
- 動態 interval：依健康度、延遲或穩定度自動調整；穩定時拉長、異常時縮短。
- 加入 jitter：同一分鐘內隨機偏移觸發時間，避免「同時瞬間」大量計算擠爆主執行緒。
- 後端節流：將批次重計算合併、降頻；避免每次事件都全量重算。

### 取捨

- 反應時間可能變慢，短暫故障較不敏感；可用「異常期縮短 interval」補足。
- 整體 CPU 壓力與排程延遲會改善，是低風險、低成本的調校方向。
...
## 解法四：Cluster / 多個 Node process 吃滿多核心
...
- JIT / runtime 設計偏向高效能，啟動速度跟執行速度都很快
...
在同樣的硬體上，這種 **純計算 + 檔案 I/O** 的腳本，用 Bun 跑起來的體感會比 Node.js 更輕更快。
...
1. **先釐清瓶頸是 I/O 還是 CPU**
   - I/O bound：Node.js 單執行緒優勢很大
   - CPU bound：一定要想辦法「丟出去」，不然 event loop 遲早被打爆

2. **把重計算抽象成服務 / 任務**
   - 不要把所有東西都堆在同一支程式裡
   - worker、cluster、sidecar 都是好選項

3. **善用多核心，不要浪費硬體**
   - Worker Threads / Cluster 可以把計算分攤到多顆 CPU
   - 老硬體特別吃這一招，因為單核性能差，但多核加總還可以

4. **Bun 適合作為「新腳本、新服務」的起點**
   - 不一定要一次把舊系統全部搬過去
   - 先從可以獨立出來的部分開始，用實際數據說話

如果你現在也在幫 Uptime Kuma 或其他 Node.js 監控系統加上負載平衡 / Failover，又剛好卡在老機器跑不動，可以從這三個方向開始檢查，也許你會跟我一樣，發現原來只是「少利用了幾顆核心」，把它們真正用起來之後，整個系統又重新順起來了。😄
