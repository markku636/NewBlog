...
Kong 是一套基於 Nginx 和 OpenResty 的高效能 API Gateway，核心以 Lua 編寫，具備反向代理、API 管理與套件管理及擴充的應用，同時也繼承了Nginx 優點，Kong 主要是PostgreSQL 來儲存設定，也支援無資料庫（DB-less）模式，本身就提供Restful api 管理非常適合 API 單一入口及API訂閱的場景。
---
