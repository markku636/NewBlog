...
category: Payment
tags: ['Google Pay','Cybersource','integrate','Fontend','NET MVC']
...
[先前也有介紹 Apple Pay on Web + Cybersource](https://blog.markkulab.net/2022/08/10/apple-pay-on-web-cybersource-integration/)  

## 程式串接前需提前準備的項目
* 付款頁需要 Https 環境 
* Google 開發者帳號( 25 美金 / 只需繳交一次 )
* 符合 [Google Pay UI 規範](https://developers.google.com/pay/api/web/guides/brand-guidelines?hl=zh-tw)
* 非直接對接需要金流商戶 (  本篇採用 cybersource )

## Google Pay 付款流程
當使用者按下Google Pay 付款按鈕，前端會從 google 得用戶端 token > 會員可以選擇可付款信用卡 > 前端會在 load 拿到 token ，帶著 token  Call 自己的後端 Api，向金流商請求建立訂單。
...
## 後面網站實作完成 Google Pay 後，依據審核要求，截圖上傳送審，經 Google 審核通過才能使用 PRODUCTION 環境。 ( 通常一兩天 )
