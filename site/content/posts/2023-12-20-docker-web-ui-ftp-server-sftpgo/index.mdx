...
## sftpgo - 官方資料
功能齊全，且高度可配置的 SFTP 伺服器，具有可選的 HTTP/S、FTP/S 和 WebDAV 支援，支援的儲存後端：本機檔案系統、加密本機檔案系統、S3（相容）物件儲存、Google 雲端儲存、Azure Blob 儲存、其他 SFTP 伺服器，並擁有 WebAdmin 及 WebClient 上傳機制。
