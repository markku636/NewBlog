...
category:  DevOps
tags:   ['Kubernetes','docker','widnows']
...
## 佈署流程
![k8s deployment](windows-k8s-deployment.png)
...
### 在Jenkins 容器中中安裝 kubectl -  kubectl: command not found
```
...
```
P.S. 切記避免錯誤 Docker Desktop 和 Jenkins 容器的 kubectl 版本最好一致
...
https://blog.csdn.net/m0_45806184/article/details/128190069
### Jenkins 透過 Kubernetes CLI 連線 kubernetes 
1. 要想把jenkins關聯到k8s中需要安裝幾個插件，打開  “系統管理” =》“管理插件” 然後安裝 Kubernetes plugin
...
pipeline {
    ...
    stages {
        ...
        stage("Deply") {
             steps {
                    withKubeConfig([credentialsId: 'secret', serverUrl: 'https://host.docker.internal:6443']) {                     
...
## 在github repo 裡 next-ec 建麼 k8s 的 YAML 佈署設定檔
```
apiVersion: v1
kind: Deployment
metadata:
...
### no matcher for kind "Deploy" in version v1 問題

yaml 改了幾次版本，請依據[官方文件](https://kubernetes.io/docs/concepts/services-networking/service/#type-nodeport)撰寫最新的 YAML 佈署檔
