...
## 簡單說明一下 GTM 
在 GTM 出現之前，如果想要在網站上新增事件追蹤，像是將數據傳送到 Google Analytics（GA）或嵌入第三方追蹤碼，都必須請工程師手動埋設程式碼。但有了 GTM 之後，這個過程變得更方便了。簡單來說，GTM 就像是 JavaScript 的後門，讓你可以在不改動網站原始碼的情況下，加入腳本或 HTML。

**GTM 是什麼？**  
GTM 是一個標籤管理工具，幫助用戶在網站或應用程式上管理和部署各種標籤（Tags），例如追蹤代碼和腳本，而無需直接修改網站的程式碼。

**GA 是什麼？**  
GA 是一個網站分析工具，用來收集並分析用戶行為數據。GA 的報表數據主要是透過 GTM 將資料轉送至 GA。

## GA4 事件

### 首先談到事件前，我們必須在網站先埋設追蹤代碼 
進入 GA4，按照以下路徑操作：
1. **設定** → **資料收集和修改** → **選擇資料串流**。
2. 點擊 **代碼設定** → **安裝操作說明** → 選擇 **手動安裝**，並將提供的程式碼嵌入網站。

![add tracking code](add-tracking-code.png.png)
...
### 步驟 2： 代碼 > 新增
![add code for load js](add-code-for-load-js)

新增一個「自訂 HTML」標籤，並加入以下程式碼：
