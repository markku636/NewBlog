...
### 分析問題
經過調查發現 3台redis 同步異常，分析硬體效能後發現這三台 Redis slave 容器在一台巢狀的 Hyper-v host中 ，依序檢檢查CPU 、記憶體、硬碟IO、且共用一張網卡，Host 被分配的記憶體只有16G ，Windows docker 最高只有 8GB的記憶體可以，且使用率都是接近滿載，推測記憶體資源不足是導致同步異常的主因，最後把那台的 Host裡的 Redis 容器實體關掉一個，讓 Host 資源能能夠被充份使用。
