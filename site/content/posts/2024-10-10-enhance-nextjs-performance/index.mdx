...
* next js 本身就會做 code spliting，app.tsx 會被包到主要的js 裡，將其子頁面，這樣就不會包在
* 只顯示頁面需要的元件 ( 這個會和seo )，頁面用不到的元件，採用動態載入元件 ( dynamic import )，元件就不會打包到主要的js 裡
* 伺服器渲染時只拿回需要的資料，不需要的資料就不要吐到用戶端。
* 並且透過 next/bundle-analyzer 分析工具得知道，那些套件過大，並找到替代的替換 ( Moment.js to Days.js )
...
(其實改成捲動後載入，效能應該會更好)
### 9. 調整網路傳輸的協定改變 (boti)、啟動協定 http 2 及 3 
### 10.減少 request 及 api reuqest 減少回傳的資量，把後端不常變動的資料 cache 起來。
### 11. 頁面渲染，能採用ISR 就用ISR ，減少在渲染使用 API，讀Redis 會比 API 來的快。
